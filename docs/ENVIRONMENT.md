...
| `Razorpay order creation failed: [object Object] {"statusCode":400,"error":{}}` | Receipt parameter exceeds 40 characters limit, or invalid environment keys | Keep receipt under 40 characters (e.g. `rcpt_<timestamp>_<random>`). Ensure `RAZORPAY_KEY_ID` in Vercel is set to live key prefix `rzp_live_*` and matched with valid `RAZORPAY_KEY_SECRET`. |
